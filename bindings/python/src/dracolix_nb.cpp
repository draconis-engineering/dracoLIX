...
NumKind num_kind(nb::handle h) noexcept {
    if (PyBool_Check(h.ptr())) return NumKind::Bool;
    if (PyLong_Check(h.ptr())) return NumKind::Int;
    if (PyFloat_Check(h.ptr())) return NumKind::Float;
    return NumKind::None;
}
...
struct DlxArray {
    using Variant =
        std::variant<dracolix::Array<float>, dracolix::Array<double>,
                     dracolix::Array<int32_t>, dracolix::Array<int64_t>,
                     dracolix::Array<bool>>;
    Variant v;
    ...
    template <typename T> explicit DlxArray(dracolix::Array<T> a)
        : v(std::move(a)) {}
    ...
    const std::vector<size_t> &shape() const {
        return std::visit([](const auto &a) -> const std::vector<size_t> & {
            return a.shape();
        }, v);
    }
...
template <typename T> struct array_scalar<dracolix::Array<T>> { using type = T; };
...
DlxArray &as_array(nb::handle h) {
    if (!is_array(h)) throw nb::type_error("expected a dracolix Array");
...
nb::object g_f32, g_f64, g_i32, g_i64, g_bool;

nb::object dtype_singleton(dracolix::DType dt) {
    switch (dt) {
        case dracolix::DType::F32: return g_f32;
        case dracolix::DType::F64: return g_f64;
        case dracolix::DType::I32: return g_i32;
        case dracolix::DType::I64: return g_i64;
        case dracolix::DType::Bool: return g_bool;
        default: throw nb::type_error("unsupported dtype");
    }
}
...
nb::object py_scalar_at(const DlxArray &a, int64_t offset) {
    return std::visit(
        [offset](const auto &arr) { return nb::cast(arr[(size_t)offset]); },
        a.v);
}
...
DlxArray scalar_slot(nb::handle h) {
    switch (num_kind(h)) {
        ...
        default: throw nb::type_error("expected a scalar");
    }
}
...
int64_t rank_of(dracolix::DType dt) {
    switch (dt) {
        case dracolix::DType::Bool: return 0;
        case dracolix::DType::I32: return 1;
        case dracolix::DType::I64: return 2;
        case dracolix::DType::F32: return 3;
        case dracolix::DType::F64: return 4;
        default: throw nb::type_error("unsupported dtype");
    }
}
...
template <typename T> DlxArray cast_cpp(const DlxArray &a) {
    return std::visit(
        [](const auto &arr) { return DlxArray(arr.template astype<T>()); },
        a.v);
}

DlxArray cast_dtype(const DlxArray &a, dracolix::DType dt) {
    switch (dt) {
        case dracolix::DType::F32: return cast_cpp<float>(a);
        case dracolix::DType::F64: return cast_cpp<double>(a);
        case dracolix::DType::I32: return cast_cpp<int32_t>(a);
        case dracolix::DType::I64: return cast_cpp<int64_t>(a);
        case dracolix::DType::Bool: return cast_cpp<bool>(a);
        default: throw nb::type_error("unsupported dtype");
    }
}
...
size_t bcast_offset(const std::vector<size_t> &shape,
    ...
    for (size_t i = 0; i < rndim; ++i) {
        ...
        if (orig_d < 0) continue;
        if (shape[orig_d] == 1) continue;
...
DlxArray bin_typed(const DlxArray &A, const DlxArray &B, BinOp op) {
    ...
    switch (op) {
        case BinOp::Add: return DlxArray(a + b);
        case BinOp::Sub: return DlxArray(a - b);
        case BinOp::Mul: return DlxArray(a * b);
        case BinOp::Div: return DlxArray(a / b);
        case BinOp::Eq: return DlxArray(compare_bcast<R>(a, b, true));
        case BinOp::Ne: return DlxArray(compare_bcast<R>(a, b, false));
    }
...
DlxArray bin_switch(dracolix::DType r, const DlxArray &A, const DlxArray &B,
                    BinOp op) {
    switch (r) {
        case dracolix::DType::F32: return bin_typed<float>(A, B, op);
        case dracolix::DType::F64: return bin_typed<double>(A, B, op);
        case dracolix::DType::I32: return bin_typed<int32_t>(A, B, op);
        case dracolix::DType::I64: return bin_typed<int64_t>(A, B, op);
        default: throw nb::type_error("unsupported dtype");
    }
}
...
nb::object dlx_neg(nb::object x) {
    if (!is_array(x)) return nb::not_implemented();
    ...
    return std::visit(
        [&](const auto &a) -> nb::object {
            ...
            return nb::cast<DlxArray>(
                DlxArray(dracolix::linalg::matmul(a, b)));
        },
        A->v);
}
...
void parse_slice_h(nb::handle obj, KeySpec &ks) {
    ...
    if (step == 0) throw nb::value_error("slice step cannot be 0");
    ks.is_int = false;
    ks.slice.step = step;
    if (start != PY_SSIZE_T_MAX) ks.slice.start = (int64_t)start;
    if (stop != PY_SSIZE_T_MIN) ks.slice.stop = (int64_t)stop;
}
...
std::vector<KeySpec> parse_key(nb::object key, size_t ndim) {
    ...
    if (out.size() > ndim) throw nb::index_error("too many indices");
    while (out.size() < ndim) out.push_back(KeySpec{}); // slice(None)
    return out;
}

nb::object dlx_scalar_index(const DlxArray &a, const std::vector<int64_t> &idx) {
    const std::vector<size_t> &shape = a.shape();
    const std::vector<size_t> &strides = a.strides();
    size_t off = 0;
    for (size_t d = 0; d < shape.size(); ++d) {
        int64_t i = idx[d];
        if (i < 0) i += (int64_t)shape[d];
...
nb::object dlx_getitem(const DlxArray &self, nb::object key) {
    ...
    bool all_int = true;
    for (auto &k : keys)
        if (!k.is_int) { all_int = false; break; }
    if (all_int) {
        std::vector<int64_t> idx;
        idx.reserve(keys.size());
        for (auto &k : keys) idx.push_back(k.idx);
...
nb::object dlx_setitem(DlxArray &self, nb::object key, nb::object val) {
    ...
    bool all_int = true;
    for (auto &ks : keys)
        if (!ks.is_int) { all_int = false; break; }
    if (!all_int)
...
const char *buffer_format(dracolix::DType dt) {
    switch (dt) {
        case dracolix::DType::F32: return "f";
        case dracolix::DType::F64: return "d";
        case dracolix::DType::I32: return "i";
        case dracolix::DType::I64: return "q";
        case dracolix::DType::Bool: return "?";
        default: return "";
    }
}
...
int dlx_getbuffer(PyObject *obj, Py_buffer *view, int flags) {
    ...
    if (ndim > 0) {
        {
            ...
            for (int i = (int)ndim - 1; i >= 0; --i) {
                if (strides[i] != expect) { is_rowmajor = false; break; }
                expect *= shape[i];
            }
        }
        {
            ...
            for (size_t i = 0; i < ndim; ++i) {
                if (strides[i] != expect) { is_colmajor = false; break; }
                expect *= shape[i];
            }
        }
    }
    ...
    if ((flags & PyBUF_F_CONTIGUOUS) == PyBUF_F_CONTIGUOUS && !is_colmajor) {
        PyErr_Format(PyExc_BufferError,
                     "dracolix array is not Fortran-contiguous");
        return -1;
    }
    ...
    if (need_shape) {
        ...
        if (!c_shape) { PyErr_NoMemory(); return -1; }
        for (size_t d = 0; d < ndim; ++d) c_shape[d] = (Py_ssize_t)shape[d];
    }
    if (need_strides) {
...
void dlx_releasebuffer(PyObject *, Py_buffer *view) {
    ...
    if (view->format) PyMem_Free(view->format);
    view->shape = nullptr;
    view->strides = nullptr;
    view->format = nullptr;
}
...
dracolix::DType infer_format_dtype(const char *fmt, Py_ssize_t itemsize) {
    if (fmt == nullptr || *fmt == 0) {
        if (itemsize == 1) return dracolix::DType::I32;
        return dracolix::DType::Void;
    }
    switch (*fmt) {
        case 'f': return dracolix::DType::F32;
        case 'd': return dracolix::DType::F64;
        case 'q':
        case 'Q':
        case 'L':
        case 'N': return dracolix::DType::I64;
        case 'i':
        case 'l':
            return itemsize >= 8 ? dracolix::DType::I64
                                 : dracolix::DType::I32;
        case 'b':
        case 'h':
        case 'B':
        case 'H':
        case 'I':
        case 'n': return dracolix::DType::I32;
        case 'e': return dracolix::DType::F32;
        case '?': return dracolix::DType::Bool;
        default: return dracolix::DType::Void;
    }
}

double read_double_at(const char *p, char tag, Py_ssize_t itemsize) {
    switch (tag) {
        case 'f': return (double)(*(const float *)p);
        case 'd': return *(const double *)p;
        case 'e': {
            uint16_t h;
            std::memcpy(&h, p, 2);
        ...
        case '?': return *(const bool *)p ? 1.0 : 0.0;
        default: {
            if (itemsize == 1) return (double)(*(const int8_t *)p);
            if (itemsize == 2) return (double)(*(const int16_t *)p);
            if (itemsize == 4) return (double)(*(const int32_t *)p);
...
int64_t read_int_at(const char *p, char tag, Py_ssize_t itemsize) {
    bool is_unsigned = (tag == 'B' || tag == 'H' || tag == 'I' || tag == 'L' ||
                        tag == 'N' || tag == 'Q');
    bool is_bool = tag == '?';
    if (is_bool) return *(const bool *)p ? 1 : 0;
    if (itemsize == 1)
    ...
    return is_unsigned ? (int64_t)(*(const uint64_t *)p)
                       : (*(const int64_t *)p);
}
...
template <> double from_raw<double>(const char *p, char tag, Py_ssize_t itemsize) {
...
template <> bool from_raw<bool>(const char *p, char tag, Py_ssize_t itemsize) {
    if (tag == '?') return *(const bool *)p;
    return read_double_at(p, tag, itemsize) != 0.0;
}

template <typename T> DlxArray buffer_to_array(const Py_buffer &view, char tag) {
    if (view.ndim == 0)
    ...
    if (n != (Py_ssize_t)out.size()) throw std::logic_error("buffer size mismatch");
...
DlxArray from_buffer(nb::object o, std::optional<dracolix::DType> dt) {
    ...
    switch (target) {
        case dracolix::DType::F32: return buffer_to_array<float>(view, tag);
        case dracolix::DType::F64: return buffer_to_array<double>(view, tag);
        case dracolix::DType::I32: return buffer_to_array<int32_t>(view, tag);
        case dracolix::DType::I64: return buffer_to_array<int64_t>(view, tag);
        case dracolix::DType::Bool: return buffer_to_array<bool>(view, tag);
        default: throw nb::type_error("unsupported dtype");
    }
}
    ...
    for (Py_ssize_t i = 0; i < len; ++i) {
        ...
        if (PyList_Check(item.ptr()) || PyTuple_Check(item.ptr())) {
            ...
            switch (num_kind(item)) {
                case NumKind::Float: any_float = true; break;
                case NumKind::Int: any_int = true; break;
                default: any_bool = true; break;
            }
        } else {
...
dracolix::DType infer_list_dtype(bool any_float, bool any_int, bool any_bool) {
    if (any_float) return dracolix::DType::F64;
    if (any_int) return dracolix::DType::I64;
    return dracolix::DType::Bool;
}

template <typename T> void fill_leaf(dracolix::Array<T> &out, size_t i, nb::object leaf);

template <> void fill_leaf<bool>(dracolix::Array<bool> &out, size_t i, nb::object leaf) {
    out[i] = nb::cast<bool>(leaf);
}
template <>
void fill_leaf<int32_t>(dracolix::Array<int32_t> &out, size_t i, nb::object leaf) {
    if (num_kind(leaf) == NumKind::Float) {
        double d = nb::cast<double>(leaf);
        if (d != std::trunc(d))
            throw nb::type_error("cannot convert non-integral float to integer dtype");
        out[i] = (int32_t)d;
    } else {
        out[i] = (int32_t)nb::cast<int64_t>(leaf);
    }
}
template <> void fill_leaf<int64_t>(dracolix::Array<int64_t> &out, size_t i, nb::object leaf) {
    if (num_kind(leaf) == NumKind::Float) {
        double d = nb::cast<double>(leaf);
        if (d != std::trunc(d))
            throw nb::type_error("cannot convert non-integral float to integer dtype");
...
template <> void fill_leaf<float>(dracolix::Array<float> &out, size_t i, nb::object leaf) {
    out[i] = (float)nb::cast<double>(leaf);
}
template <> void fill_leaf<double>(dracolix::Array<double> &out, size_t i, nb::object leaf) {
...
DlxArray dlx_array(nb::object data, nb::object dtype) {
    ...
    if (PyObject_CheckBuffer(data.ptr())) return from_buffer(data, dt);
...
nb::object dlx_asarray(nb::object data, nb::object dtype) {
    ...
    if (is_array(data)) {
        if (dt && *dt != nb::cast<DlxArray *>(data)->dtype())
            return nb::cast<DlxArray>(
                cast_dtype(*nb::cast<DlxArray *>(data), *dt));
        return data; // no copy
    }
...
std::vector<size_t> parse_shape(nb::object o) {
    ...
    if (PyLong_Check(o.ptr())) {
        ...
        if (n <= 0) throw nb::value_error("dimensions must be positive");
...
template <>
int32_t scalar_to_dtype<int32_t>(nb::handle h) {
...
template <>
int64_t scalar_to_dtype<int64_t>(nb::handle h) {
    double d = nb::cast<double>(h);
    if (d != std::trunc(d))
...
template <typename T> DlxArray make_filled(const std::vector<size_t> &shape, T v) {
...
DlxArray dlx_arange(nb::object start, nb::object stop, nb::object step) {
    ...
    for (nb::handle h : {nb::handle(a0), nb::handle(a1), nb::handle(a2)})
        if (num_kind(h) == NumKind::Float) any_float = true;
    ...
    if (!any_float) {
        ...
        int64_t span = s1 - s0;
        int64_t n64 = 0;
        if (s2 > 0 && span > 0) n64 = (span + s2 - 1) / s2;
        if (s2 < 0 && span < 0) n64 = (span + s2 + 1) / s2;
        if (n64 < 0) n64 = 0;
        dracolix::Array<int64_t> a(std::vector<size_t>{(size_t)n64});
        for (int64_t i = 0; i < n64; ++i) a[(size_t)i] = s0 + i * s2;
        return DlxArray(std::move(a));
    }
    ...
    int64_t n64 = 0;
    if (d2 != 0.0) {
        if (d2 > 0 && span > 0) n64 = (int64_t)std::ceil(span / d2);
        if (d2 < 0 && span < 0) n64 = (int64_t)std::ceil(span / d2);
    }
    if (n64 < 0) n64 = 0;
    dracolix::Array<double> a(std::vector<size_t>{(size_t)n64});
    for (int64_t i = 0; i < n64; ++i) a[(size_t)i] = d0 + (double)i * d2;
    return DlxArray(std::move(a));
}
...
struct FlatIter {
    ...
    int64_t i = 0;
    FlatIter &operator++() { ++i; return *this; }
...
DlxArray dlx_min_axis(const DlxArray &a, int64_t axis) {
    axis = normalize_axis(a, axis);
    return std::visit(
        [&](const auto &arr) -> DlxArray { return DlxArray(arr.min_axis((size_t)axis)); },
        a.v);
}
...
DlxArray dlx_max_axis(const DlxArray &a, int64_t axis) {
    axis = normalize_axis(a, axis);
    return std::visit(
        [&](const auto &arr) -> DlxArray { return DlxArray(arr.max_axis((size_t)axis)); },
        a.v);
}
...
DlxArray dlx_mean_axis(const DlxArray &a, int64_t axis) {
    axis = normalize_axis(a, axis);
    return std::visit(
        [&](const auto &arr) -> DlxArray { return DlxArray(arr.mean_axis((size_t)axis)); },
        a.v);
}
...
DlxArray dlx_reshape(const DlxArray &a, nb::object shape) {
    ...
    return std::visit(
        [&](const auto &arr) -> DlxArray {
            dracolix::Array<typename array_scalar<std::decay_t<decltype(arr)>>::type>
                c = arr;
            c.reshape(s);
...
DlxArray dlx_transpose(const DlxArray &a) {
    if (a.ndim() != 2) throw nb::value_error("transpose() requires 2-D");
    return std::visit(
        [](const auto &arr) { return DlxArray(arr.transpose()); },
        a.v);
}

DlxArray dlx_copy(const DlxArray &a) {
    return std::visit([](const auto &arr) { return DlxArray(arr.clone()); },
                      a.v);
}

DlxArray dlx_astype(const DlxArray &a, nb::object dtype) {
    std::optional<dracolix::DType> dt = optional_dtype(dtype);
    if (!dt) throw nb::type_error("astype() requires a dtype");
...
NB_MODULE(_dracolix_nb, m) {
    ...
    nb::class_<DlxDType>(m, "DType")
        .def("__eq__",
             [](const DlxDType &a, const nb::object &other) -> nb::object {
                 if (!is_dtype(other)) return nb::not_implemented();
        ...
        .def("__repr__", [](const DlxDType &a) {
            return "dtype('" + dracolix::dtype_name(a.dt) + "')";
        })
        .def_prop_ro("name", [](const DlxDType &a) {
            return dracolix::dtype_name(a.dt);
        });

    nb::class_<DlxArray> arr_cls(m, "Array");
    arr_cls
        .def_prop_ro("dtype", [](const DlxArray &a) { return dtype_singleton(a.dtype()); })
        .def_prop_ro("shape", [](const DlxArray &a) {
            nb::tuple t = nb::steal<nb::tuple>(PyTuple_New((Py_ssize_t)a.ndim()));
            for (size_t d = 0; d < a.ndim(); ++d)
                PyTuple_SET_ITEM(t.ptr(), (Py_ssize_t)d,
                                 nb::int_((int64_t)a.shape()[d]).release().ptr());
            return t;
        })
        .def_prop_ro("strides", [](const DlxArray &a) {
            nb::tuple t = nb::steal<nb::tuple>(PyTuple_New((Py_ssize_t)a.ndim()));
            for (size_t d = 0; d < a.ndim(); ++d)
                PyTuple_SET_ITEM(t.ptr(), (Py_ssize_t)d,
        ...
        .def_prop_ro("ndim", [](const DlxArray &a) { return (Py_ssize_t)a.ndim(); })
        .def_prop_ro("size", [](const DlxArray &a) { return (Py_ssize_t)a.size(); })
        .def_prop_ro("itemsize", [](const DlxArray &a) { return (Py_ssize_t)a.itemsize(); })
        .def_prop_ro("layout", [](const DlxArray &a) {
            return a.layout() == dracolix::Layout::RowMajor ? "row-major"
                                                            : "col-major";
        })
        .def_prop_ro("T", [](const DlxArray &a) -> DlxArray {
            if (a.ndim() != 2) throw nb::value_error(".T requires 2-D");
            return dlx_transpose(a);
        })

        .def("__len__", [](const DlxArray &a) {
            return (Py_ssize_t)(a.ndim() ? a.shape()[0] : 1);
        })
        .def("__bool__", [](const DlxArray &a) -> bool {
            if (a.size() != 1)
                throw nb::type_error("the truth value of an array with more "
                                     "than one element is ambiguous");
            return std::visit([](const auto &arr) -> bool { return (bool)arr[0]; },
                              a.v);
        })
        .def("__iter__",
        ...
        .def("sum", [](const DlxArray &a, int64_t axis) { return dlx_sum_axis(a, axis); },
             nb::arg("axis"))
        .def("min", [](const DlxArray &a) { return dlx_min(a); })
        .def("min", [](const DlxArray &a, int64_t axis) { return dlx_min_axis(a, axis); },
             nb::arg("axis"))
        .def("max", [](const DlxArray &a) { return dlx_max(a); })
        .def("max", [](const DlxArray &a, int64_t axis) { return dlx_max_axis(a, axis); },
             nb::arg("axis"))
        .def("mean", [](const DlxArray &a) { return dlx_mean(a); })
        .def("mean", [](const DlxArray &a, int64_t axis) { return dlx_mean_axis(a, axis); },
             nb::arg("axis"))
        ...
        .def("__rtruediv__", [](nb::object x, nb::object y) { return dlx_div(y, x); })
        .def("__neg__", &dlx_neg)
        .def("__eq__", &dlx_eq)
        .def("__ne__", &dlx_ne)
        .def("__matmul__", &dlx_matmul)
        .def("__rmatmul__", [](nb::object x, nb::object y) { return dlx_matmul(y, x); });

    // singletons (must be created after class registrations)
    g_f32 = nb::cast<DlxDType>(DlxDType(dracolix::DType::F32));
    g_f64 = nb::cast<DlxDType>(DlxDType(dracolix::DType::F64));
    g_i32 = nb::cast<DlxDType>(DlxDType(dracolix::DType::I32));
    g_i64 = nb::cast<DlxDType>(DlxDType(dracolix::DType::I64));
    g_bool = nb::cast<DlxDType>(DlxDType(dracolix::DType::Bool));
    m.attr("f32") = g_f32;
    m.attr("f64") = g_f64;
    m.attr("i32") = g_i32;
    m.attr("i64") = g_i64;
    ...
    m.def("asarray", &dlx_asarray, nb::arg("data"), nb::arg("dtype") = nb::none());
