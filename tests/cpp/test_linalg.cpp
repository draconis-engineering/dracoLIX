...
void test_matmul() {
	...
	assert(C.at(0, 0) == 58 && C.at(0, 1) == 64 && C.at(1, 0) == 139 &&
		   C.at(1, 1) == 154);
	std::cout << " matmul ok\n";
}
void test_matvec() {
...
void test_diag() {
	...
	for (size_t i = 0; i < 9; ++i)
		A[i] = i;
	auto d = diagonal(A);
	assert(d[0] == 0 && d[1] == 4 && d[2] == 8);
	auto M = diag(d);
	assert(M.at(1, 1) == 4 && M.at(0, 1) == 0);
	std::cout << " diag ok\n";
}
void test_batched() {
	Array<double> A({2, 2, 3});
	Array<double> B({2, 3, 2});
	...
	// 4D
	Array<double> A4({2, 2, 2, 3});
	for (size_t i = 0; i < A4.size(); ++i)
		A4[i] = 1;
	Array<double> B4({2, 2, 3, 2});
