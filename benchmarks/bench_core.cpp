...
void fill_rand(Array<T>& a, uint64_t seed = 0x9e3779b97f4a7c15ULL) {
    ...
    for (size_t i = 0; i < a.size(); ++i) a[i] = static_cast<T>(dist(rng));
}
...
int main(int argc, char** argv) {
    ...
    if (argc > 1) iters_small = std::stoul(argv[1]);
    ...
    // --- elementwise (no broadcast) ---
    {
        ...
        auto r = bench::run("add (1M)", "1024x1024", iters_small, 5, [&]{
            volatile auto c = a + b; (void)c;
    ...
    }
    {
        ...
        fill_rand(a, 3); fill_rand(big, 5);
        auto r = bench::run("add broadcast", "2048+2048x2048", 100, 5, [&]{
            volatile auto c = big + a; (void)c;
        }, 2048*2048);
        bench::print_result(r);
    }
    ...
    // --- reductions ---
    {
        ...
        fill_rand(a, 6);
        auto r = bench::run("sum 1M", "1M", iters_small, 5, [&]{
            volatile auto s = a.sum(); (void)s;
        }, a.size());
        bench::print_result(r);
    }
    {
        ...
        fill_rand(a, 7);
        auto r = bench::run("sum axis=0", "1024x1024", 100, 5, [&]{
            volatile auto s = a.sum(0); (void)s;
        }, a.size());
        bench::print_result(r);
    }
    ...
    for (size_t n : {64, 256, 512}) {
        ...
        fill_rand(A, n); fill_rand(B, n+100);
        std::string shape = std::to_string(n) + "x" + std::to_string(n);
        size_t flops = 2 * n * n * n; // multiply-add = 2 FLOPs
        auto r = bench::run("matmul", shape, iters_gemm, 2, [&]{
            volatile auto C = matmul(A, B); (void)C;
        }, flops);
        bench::print_result(r);
    }
    ...
    // --- batched matmul (atleast-3D) ---
    {
        ...
        fill_rand(A, 10); fill_rand(B, 11);
        auto r = bench::run("batched matmul", "8x128x128", 20, 2, [&]{
            volatile auto C = matmul(A, B); (void)C;
    ...
    }
    {
        ...
        fill_rand(A, 12); fill_rand(B, 13);
        auto r = bench::run("batched matmul", "4x256x64*64x256", 20, 2, [&]{
            volatile auto C = matmul(A, B); (void)C;
    ...
    // --- sparse matvec ---
    {
        ...
        for (size_t i=0;i<dense.size();++i) dense[i] = (sparsity(rng) < 0.01 ? d(rng) : 0.0);
        auto csr = CsrMatrix<double>::from_dense(dense);
        Array<double> x({1024}); fill_rand(x, 20);
        size_t flops = csr.nnz()*2;
        auto r = bench::run("csr matvec 1%", "1024x1024", 200, 5, [&]{
            volatile auto y = csr.matvec(x); (void)y;
        }, flops);
        bench::print_result(r);
    ...
    {
        Array<double> A({1024,1024}); fill_rand(A, 30);
        auto r1 = bench::run("transpose_view", "1024x1024", 5000, 100, [&]{
            volatile auto v = A.transpose_view(); (void)v;
        });
        bench::print_result(r1);
        auto r2 = bench::run("transpose copy", "1024x1024", 50, 5, [&]{
            volatile auto B = A.transpose(); (void)B;
        }, A.size());
