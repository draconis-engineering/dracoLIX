...
template <typename T> class Array {
  public:
	explicit Array(size_t size, Layout layout = Layout::RowMajor)
		...
		if (dtype_ == DType::Void)
			throw std::logic_error("Array<T>: unmapped DType");
		data_.resize(size_);
		compute_strides();
	}
	...
	explicit Array(std::vector<size_t> shape, Layout layout = Layout::RowMajor)
		...
		if (dtype_ == DType::Void)
			throw std::logic_error("Array<T>: unmapped DType");
		if (shape_.empty())
			throw std::invalid_argument("Array: shape cannot be empty");
		ndim_ = shape_.size();
		size_ = 1;
		for (auto s : shape_) {
			if (s == 0)
				throw std::invalid_argument("Array: dimension cannot be 0");
			size_ *= s;
		}
		data_.resize(size_);
		compute_strides();
	...
	T &at(size_t i, size_t j) {
		if (ndim_ != 2)
			throw std::logic_error("at(i,j) requires ndim==2");
		if (i >= shape_[0] || j >= shape_[1])
			throw std::out_of_range("at(i,j) out of bounds");
		return reinterpret_cast<T &>(data_[i * strides_[0] + j * strides_[1]]);
	}
	const T &at(size_t i, size_t j) const {
		return const_cast<Array *>(this)->at(i, j);
	}
	...
	const T &at(const std::vector<size_t> &indices) const {
		return const_cast<Array *>(this)->at(indices);
	}

	// Element-wise ops - return new array
	Array operator+(const Array &o) const {
		return elementwise(o, [](T a, T b) { return a + b; });
	}
	Array operator-(const Array &o) const {
		return elementwise(o, [](T a, T b) { return a - b; });
	}
	...
	Array operator+(T scalar) const {
		return scalar_op(scalar, [](T a, T s) { return a + s; });
	}
	Array operator-(T scalar) const {
		return scalar_op(scalar, [](T a, T s) { return a - s; });
	}
	...
	ArrayView<T> slice(Slice s) {
		if (ndim_ != 1)
			throw std::logic_error("slice(Slice) requires 1-D");
		size_t off;
		size_t len;
		int64_t step;
	...
	ArrayView<T> slice(const std::vector<Slice> &slices) {
		if (slices.size() != ndim_)
			throw std::invalid_argument("slice: rank mismatch");
		std::vector<size_t> nshape;
		nshape.reserve(ndim_);
		std::vector<size_t> nstrides;
		nstrides.reserve(ndim_);
		size_t base_off = 0;
		for (size_t d = 0; d < ndim_; ++d) {
			size_t off, len;
			int64_t step;
	...
	ArrayView<T> transpose_view() {
		if (ndim_ != 2)
			throw std::logic_error("transpose_view requires 2-D");
	...
	Array transpose() const {
		if (ndim_ != 2)
			throw std::logic_error("transpose requires 2-D");
	...
	// ---- Type conversion ----
	template <typename U> Array<U> astype() const {
		Array<U> out(shape_);
	...
	T min() const {
		...
		for (size_t t = 0; t < nthreads; ++t) {
			...
			if (s >= e)
				break;
			if (t == 0)
				s = 0; // ensure first chunk covers data_[0]
			futs.push_back(ThreadPool::global().enqueue([&, t, s, e] {
				T m = static_cast<T>(data_[s]);
				for (size_t i = s + 1; i < e; ++i)
					if (static_cast<T>(data_[i]) < m)
						m = static_cast<T>(data_[i]);
				partials[t] = m;
			}));
		}
		for (auto &f : futs)
  ...
  private:
	...
	Array reduce_axis(size_t axis, T (*op)(T, T), T init) const {
		...
		if (out_shape.empty())
			out_shape = {1}; // keep at least 1D for scalar? alternative: return
							 // 1-element
		// But for true scalar reduction we want size 1; caller sum(axis) on 1-D
		// -> scalar array
		Array out(out_shape);
