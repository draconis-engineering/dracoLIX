...
private:
  ...
  Array reduce_axis(size_t axis, T (*op)(T, T), T init) const {
    ...
    if (out_shape.empty())
      out_shape = {
          1}; // keep at least 1D for scalar? alternative: return 1-element
  ...
  Array reduce_axis_minmax(size_t axis, bool is_min) const {
    ...
    for (size_t flat = 0; flat < size_; ++flat) {
      ...
      if (idx[axis] == 0)
        ...
        if (is_min)
          out.data()[out_flat] =
              std::min<T>(out.data()[out_flat], static_cast<T>(data_[flat]));
        else
          out.data()[out_flat] =
              std::max<T>(out.data()[out_flat], static_cast<T>(data_[flat]));
      }
    }
    return out;
  }
