...
template <typename T> class ArrayView {
  public:
	...
	ArrayView(T *data, std::vector<size_t> shape, std::vector<size_t> strides)
		...
		if (shape_.size() != strides_.size()) {
			throw std::invalid_argument(
				"ArrayView: shape/strides size mismatch");
		}
		ndim_ = shape_.size();
		size_ = 1;
		for (auto s : shape_) {
	...
	T &operator[](size_t flat) {
		if (flat >= size_)
			throw std::out_of_range("ArrayView flat index OOB");
		// For flat access we need to map flat -> multidimensional using
		// shape/strides? Simpler: if contiguous (strides are row-major), flat
		// is direct offset. For strided views (slice), flat iteration is still
		// logical, not physical. We implement logical flat via unravel.
		auto idx = unravel(flat);
		return at_indices(idx);
	}
	const T &operator[](size_t flat) const {
...
struct Slice {
	...
	static Slice from(int64_t start) {
		Slice s;
		s.start = start;
		return s;
	}
	static Slice to(int64_t stop) {
		Slice s;
		s.stop = stop;
		return s;
	}
...
inline void normalize_slice(const Slice &s, size_t dim, size_t &out_offset,
							size_t &out_len, int64_t &out_step) {
	...
	if (step > 0) {
		...
		if (start <= stop)
			...
			out_len = (size_t)((start - stop - 1) / (-step) + 1);
		out_offset =
			(start >= 0 ? (size_t)start : 0); // if len==0 offset irrelevant
		if (out_len == 0)
			out_offset = 0;
	}
	out_step = step;
