...
inline SymEigenResult eig_sym(const Array<double>& A, double tol = 1e-10, int max_sweeps = 100) {
    if (A.ndim() != 2) throw std::invalid_argument("eig_sym requires 2-D");
    size_t n = A.shape()[0];
    if (A.shape()[1] != n) throw std::invalid_argument("eig_sym requires square");
    ...
    Array<double> vals({n});
    for (size_t i = 0; i < n; ++i) vals[i] = D.at(i, i);
    ...
    std::sort(idx.begin(), idx.end(), [&](size_t a, size_t b){ return vals[a] > vals[b]; });
    Array<double> svals({n});
    Array<double> svecs({n, n});
    for (size_t j = 0; j < n; ++j) {
        svals[j] = vals[idx[j]];
        for (size_t i = 0; i < n; ++i) svecs.at(i, j) = V.at(i, idx[j]);
    }
