...
inline GemmKernel select_gemm_kernel(size_t m, size_t n, size_t p) {
	...
	size_t ops = m * n * p;
	if (ops < 4096)
		return GemmKernel::Scalar; // small: scalar wins (no dispatch overhead)

	static const auto feats = cpu::detect();
	// Prefer widest available; payload kernels probe at runtime via
	// ifunc/builtin
	if (feats.avx512f)
	...
	if (feats.avx)
		return GemmKernel::Avx2; // fallback
