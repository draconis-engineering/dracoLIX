...
// ---- SIMD variants (compile-time specialization, runtime dispatch checks feature) ----
#if defined(__AVX2__)
static void gemm_f64_avx2(const double* A, const double* B, double* C, size_t m, size_t n, size_t p) {
...
static void gemm_f32_avx2(const float* A, const float* B, float* C, size_t m, size_t n, size_t p) {
    for (size_t i=0;i<m;++i) {
        for (size_t k=0;k<n;++k) {
...
void dispatch_gemm_f64(const double* A, const double* B, double* C, size_t m, size_t n, size_t p) {
    ...
    if (k == GemmKernel::Avx2 || k == GemmKernel::Avx512) {
        ...
        #if defined(__GNUC__)
        if (__builtin_cpu_supports("avx2")) { gemm_f64_avx2(A,B,C,m,n,p); return; }
        #endif
    }
#endif
    (void)k;
...
void dispatch_gemm_f32(const float* A, const float* B, float* C, size_t m, size_t n, size_t p) {
    ...
    if (k == GemmKernel::Avx2 || k == GemmKernel::Avx512) {
        #if defined(__GNUC__)
        if (__builtin_cpu_supports("avx2")) { gemm_f32_avx2(A,B,C,m,n,p); return; }
        #endif
    }
#endif
    (void)k;
